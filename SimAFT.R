...
## ????????ʽ?任
changeNotes <- function(x, n, p, totaln = n, m = 1){
  ind <- m * seq(0, p - 1)
  ind3 <- cumsum(n)
...
data.gener <- function(n, p, q, q1, np.ind, np.perc, beta.true, alpha.true, censor.rate){
  ...
  IXX <- changeNotes(IX, n, p*(p-1)/2, totaln, m)
  fU <- fU%*%diag(np.ind)
  # XX: changed X
  # X: original X
  XX <- changeNotes(X, n, p, totaln, m)
  true.time <- drop(XX %*% beta.true + rowSums(fU) + IXX %*% alpha.true + err)
  cencor <- runif(totaln, max = censor.rate * max(true.time))
  theta <- ifelse(true.time <= cencor, 1, 0)
  Y <- censor
  Y[theta] <- true.time[theta]
  data <- list(Y = Y, X = X, XX = XX, IX = IX, IXX = IXX, U = U, fU = fU, theta = theta)
  return(list(data = data, Y = Y, X = X, XX = XX, U = U, IX = IX, IXX = IXX, fU = fU, n = n,
              p = p, q = q, q1 = q1, beta.true = beta.true, alpha.true = alpha.true, theta = theta))
}
data.regener <- function(data, n, p){
  Y <- data$Y; X <- data$X;  U <- data$U; fU <- data$fU;  IX <- data$IX; theta <- data$theta
  n <- cumsum(c(0, n))
  newdata <- list(Y = Y[(n[p] + 1):n[p + 1]], XX = X[(n[p] + 1):n[p + 1], ], 
                  IXX = IX[(n[p] + 1):n[p + 1], ], U = U[(n[p] + 1):n[p + 1], ], 
                  fU = fU[(n[p] + 1):n[p + 1], ], theta = theta[(n[p] + 1):n[p + 1]])
  return(newdata)
}

WeightFun <- function(Y, weight){
  Y <- sqrt(weight) * (Y - cumsum(weight * Y) / cumsum(weight))
  return(Y)
}

SortData <- function(data, n){
  Y <- data$Y; X <- data$X;  U <- data$U; fU <- data$fU;  IX <- data$IX; theta <- data$theta
  ind <- sort.int(Y, index.return = TRUE)$ix
  Y <- sort(Y); X <- X[ind, ]; U <- U[ind, ]; fU <- fU[ind, ]; theta <- theta[ind]
  weights <- NULL
  weights[1] <- theta[1] / n
  for(i in 2:n){
    temp <- 1
    for(j in 1:(i - 1)){
      temp <- temp * ((n - j) / (n - j + 1)) ^ theta[j]
    }
    weights[i] <- theta[i] / (n - i + 1) * temp
  }
  Y <- WeightFun(Y, weights)
  X <- apply(X, 1, WeightFun, weight = weights)
  U <- apply(U, 1, WeightFun, weight = weights)
  fU <- apply(fU, 1, WeightFun, weight = weights)
  IX <- get.interac(X)
  IXX <- changeNotes(IX, n, p*(p-1)/2, totaln = n)
  XX <- changeNotes(X, n, p, totaln = n)
  data <- list(Y = Y, X = X, XX = XX, IX = IX, IXX = IXX, U = U, fU = fU, theta = theta)
  return(data)
}
...
est.perf <- function(data, n, N.bs = seq(4, 12, 4), beta.true = NA, alpha.true = NA, case = 1){
  ...
  if (case != 1){
    ...
    MSE <- 3 * crossprod(beta.adjust - beta.true)/length(beta.true)
    MSE2 <- 3 * crossprod(alpha.adjust - alpha.true)/length(alpha.true)
  } else {
    MSE <- 3 * crossprod(beta.est - beta.true)/length(beta.true)
    MSE2 <- 3 * crossprod(alpha.est - alpha.true)/length(alpha.true)
  }
...
q1 <- 2
m <- 3
beta.true <- c(1, 2, 3, 3, 1, 2, 2, 3, 1, rep(0, m * 3))
alpha.true <- c(0.3, 2, 0.5, 1, 0.5, 0.3, rep(0, m * 5 * 3 - 6))

dir.create(paste("./sim", run, "_p", p, sep = ''), showWarnings = FALSE)
setwd(paste("./sim", run, "_p", p, sep = ''))
...
sim.run <- function(run, n, p, q, q1, beta.true, alpha.true){
  ...
  for (i in 1:run){
    ...
    testdata <- SortData(my$data, sum(n))
    result2 <- est.perf(testdata, n, N.bs = seq(4, 24, 4), beta.true, alpha.true) #M2
    ...
    N.bs[i] <- result2$N.bs
    sepdata <- list(Y = testdata$Y, X = testdata$X, IX = testdata$IX, U = testdata$U, fU = testdata$fU)
    sepdata3 <- list(Y = testdata$Y, XX = testdata$X, IXX = testdata$IX, U = testdata$U, fU = testdata$fU)
    testdata3 <- SortData(sepdata3, sum(n))
    result3 <- est.perf(testdata3, sum(n), N.bs = seq(4, 24, 4), beta.true = beta.true, alpha.true = alpha.true,
                        case = 3) #M3
    ...
    for(t in 1:length(n)){    #M1
      ...
      testdata2 <- SortData(newdata, n[t])
      result <- est.perf(testdata2, n[t], N.bs = seq(4, 24, 4), beta.trues[t, ], alpha.trues[t, ])
  ...
  return(list(beta.list = beta.list, beta = beta, alpha.list = alpha.list, alpha = alpha,
              fU = fU, Err.list = Err.list, Err = Err, N.bs = N.bs, 
              beta.lists = beta.lists, betas = betas, alpha.lists = alpha.lists, alphas = alphas,
              Err.lists = Err.lists, Errs = Errs, N.bss = N.bss, 
